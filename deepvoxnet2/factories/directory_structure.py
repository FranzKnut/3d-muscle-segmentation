...
class LotteStructure(Structure):
    def __init__(self, base_dir, experiment_name, fold_i, training_identifiers=None, validation_identifiers=None, testing_identifiers=None):
        self.base_dir = base_dir
        self.run_name = experiment_name
        self.run_dir = os.path.join(self.base_dir, self.run_name)
        self.fold_i = fold_i
        self.fold_dir = os.path.join(self.base_dir, self.run_name, 'Fold_{}'.format(self.fold_i))
        super(LotteStructure, self).__init__(
            logs_dir=os.path.join(self.run_dir, 'Logs', 'Fold_{}'.format(self.fold_i)),
            models_dir=os.path.join(self.run_dir, 'Models', 'Fold_{}'.format(self.fold_i)),
            history_path=os.path.join(self.run_dir, 'Models', 'Fold_{}'.format(self.fold_i), 'training_result.pkl'),
            train_images_output_dirs=[os.path.join(self.run_dir, 'Images', 'Training', 'Fold_{}'.format(self.fold_i), '{}-{}'.format(identifier.case_id, identifier.record_id.split('visit_')[1])) for
                                      identifier in training_identifiers] if training_identifiers is not None else None,
            val_images_output_dirs=[os.path.join(self.run_dir, 'Images', 'Validation', 'Fold_{}'.format(self.fold_i), '{}-{}'.format(identifier.case_id, identifier.record_id.split('visit_')[1])) for
                                      identifier in validation_identifiers] if validation_identifiers is not None else None,
            test_images_output_dirs=[os.path.join(self.run_dir, 'Images', 'Testing', 'Fold_{}'.format(self.fold_i), '{}-{}'.format(identifier.case_id, identifier.record_id.split('visit_')[1])) for
                                      identifier in testing_identifiers] if testing_identifiers is not None else None,

...
class RandomDixonImage(Transformer):
    "Generate a random image I as a combination of IP, OP, Fat and Water Dixon Images"
    def __init__(self, reference_connection, **kwargs):
        super(RandomDixonImage, self).__init__(extra_connections=reference_connection, **kwargs)
        self.reference_connection = reference_connection

    def _update_idx(self, idx):
        for idx_ in range(len(self.connections[idx][0])):
            self.outputs[idx][idx_] = Sample(sum([random.randrange(-1000, 1000)/1000*(np.multiply(connection[idx_], self.reference_connection[0])) for connection in self.connections[idx]]), self.connections[idx][0][idx_].affine)
...
class NormalizeImage(Transformer):
    """Normalization to zero mean and a standard deviation of 1 of the input image or patch (sample)"""
    def __init__(self, **kwargs):
        super(NormalizeImage, self).__init__(**kwargs)

    def _update_idx(self, idx):
        for idx_, sample in enumerate(self.connections[idx][0]):
            self.outputs[idx][idx_] = Sample((sample - sample.mean()) /sample.std(), sample.affine)
        ...
        if self.n_ == 0:
            ...
            for class_ in range(nb_classes):
                ...
                if self.nonzero:
                    ...
                    coordinates = [tuple([random.choice(range(reference.shape[i])) for i in range(1, 4)]) for _ in range(np.prod(reference.shape[1:4]) if self.ncrops is None else n_crops_per_class)] # not tested so could be that it does not work
                if class_ == 0:
...
class RandomCropFullImage_z(Crop):
    def __init__(self, reference_connection, segment_size, nonzero=False, grid_size=None, subsample_factors=(1, 1, 1), default_value=0, prefilter=None, **kwargs):
        super(RandomCropFullImage_z, self).__init__(reference_connection, segment_size, subsample_factors, default_value, prefilter, **kwargs)
        self.nonzero = nonzero
    ...
    def _randomize(self):
        if self.n_ == 0:
            assert all([np.array_equal(self.reference_connection[0].shape[1:4], sample.shape[1:4]) for connection in self.connections for sample in connection[0] if sample is not None])
            if self.nonzero:
                mask_data_ = copy.deepcopy(np.ones(self.reference_connection[0][0, :, :, :, 0].shape[2]))

                def ceiling_division(n, d=2):
                    return -(n // -d)

                x_coordinate = self.reference_connection[0].shape[1] // 2
                y_coordinate = self.reference_connection[0].shape[2] // 2

                middle_points = []
                while np.any(mask_data_):
                    z_coordinates = list(zip(*np.nonzero(np.any(np.expand_dims(mask_data_, axis=[0, -1]) != 0, axis=(0, -1)))))
                    rc = random.choice(z_coordinates)
                    z_min = rc[0] - self.grid_size[2] // 2 if rc[0] - self.grid_size[2] // 2 > 0 else 0
                    z_max = rc[0] + ceiling_division(self.grid_size[2]) if rc[0] + ceiling_division(self.grid_size[2]) <  mask_data_.shape[0] else mask_data_.shape[0]
                    mask_data_[z_min:z_max] = 0
                    middle_points.append((x_coordinate, y_coordinate, rc[0]))

                self.coordinates = middle_points

            else:
                # TODO: did not adjust this part --> thus still just provides a random cropping
                self.coordinates = [tuple([random.choice(range(self.reference_connection[0].shape[i])) for i in range(1, 4)]) for _ in range(np.prod(self.reference_connection[0].shape[1:4]) if self.ncrops is None else self.ncrops)]
...
class RandomCropFullImage(Crop):
    def __init__(self, reference_connection, segment_size, nonzero=False, grid_size=None, subsample_factors=(1, 1, 1), default_value=0, prefilter=None, **kwargs):
        super(RandomCropFullImage, self).__init__(reference_connection, segment_size, subsample_factors, default_value, prefilter, **kwargs)
        self.nonzero = nonzero
        self.grid_size = segment_size if grid_size is None else grid_size
    ...
    def _randomize(self):
        if self.n_ == 0:
            assert all([np.array_equal(self.reference_connection[0].shape[1:4], sample.shape[1:4]) for connection in self.connections for sample in connection[0] if sample is not None])
            if self.nonzero:
                mask_data_ = copy.deepcopy(self.reference_connection[0][0, :, :, :, 0])
                ...
                middle_points = []
                while np.any(mask_data_):
                    coordinates = list(zip(*np.nonzero(np.any(np.expand_dims(mask_data_, axis=[0, -1]) != 0, axis=(0, -1)))))
                    if len(coordinates) > self.grid_size[0] * self.grid_size[1] * self.grid_size[2]:
                        nb_crops = 10
                    elif len(coordinates) > self.grid_size[0] * self.grid_size[1] * self.grid_size[2] / 4:
                        nb_crops = 2
                    else:
                        nb_crops = 1

                    rcs = [random.choice(coordinates) for _ in range(nb_crops)]

                    for rc in rcs:
                        x_min = rc[0] - self.grid_size[0] // 2 if rc[0] - self.grid_size[0] // 2 > 0 else 0
                        x_max = rc[0] + ceiling_division(self.grid_size[0]) if rc[0] + ceiling_division(self.grid_size[0]) < mask_data_.shape[0] else mask_data_.shape[0]
                        y_min = rc[1] - self.grid_size[1] // 2 if rc[1] - self.grid_size[1] // 2 > 0 else 0
                        y_max = rc[1] + ceiling_division(self.grid_size[1]) if rc[1] + ceiling_division(self.grid_size[1]) < mask_data_.shape[1] else mask_data_.shape[1]
                        z_min = rc[2] - self.grid_size[2] // 2 if rc[2] - self.grid_size[2] // 2 > 0 else 0
                        z_max = rc[2] + ceiling_division(self.grid_size[2]) if rc[2] + ceiling_division(self.grid_size[2]) <  mask_data_.shape[2] else mask_data_.shape[2]
                        mask_data_[x_min:x_max, y_min:y_max, z_min:z_max] = 0
                        middle_points.append(rc)
...
class RandomCrop_z(Crop):
    def __init__(self, reference_connection, segment_size, nonzero=False, subsample_factors=(1, 1, 1), default_value=0,
                 prefilter=None, **kwargs):
        super(RandomCrop_z, self).__init__(reference_connection, segment_size, subsample_factors, default_value,
                                           prefilter, **kwargs)
        self.nonzero = nonzero

    def _randomize(self):
        if self.n_ == 0:
            assert all([np.array_equal(self.reference_connection[0].shape[1:4], sample.shape[1:4]) for connection in
                        self.connections for sample in connection[0] if sample is not None])
            if self.nonzero:
                x_coordinates = [self.connections[0][0][0].shape[1] // 2] * self.n
                y_coordinates = [self.connections[0][0][0].shape[2] // 2] * self.n
                pos_z_coordinates = list(
                    zip(*np.nonzero(np.any(self.reference_connection[0] != 0, axis=(0, 1, 2, -1)))))
                z_coordinates = [random.choice(pos_z_coordinates)[0] for _ in range(self.n)]
                self.coordinates = [(x, y, z) for x, y, z in zip(x_coordinates, y_coordinates, z_coordinates)]

            else:
                x_coordinates = [self.connections[0][0][0].shape[1] // 2] * self.n
                y_coordinates = [self.connections[0][0][0].shape[2] // 2] * self.n
                z_coordinates = [random.choice(range(self.reference_connection[0].shape[3])) for _ in range(self.n)]
                self.coordinates = [(x, y, z) for x, y, z in zip(x_coordinates, y_coordinates, z_coordinates)]


class GridCrop_z(Crop):
    def __init__(self, reference_connection, segment_size, n=None, grid_size=None, strides=None, nonzero=False,
                 subsample_factors=(1, 1, 1), default_value=0, prefilter=None, **kwargs):
        super(GridCrop_z, self).__init__(reference_connection, segment_size, subsample_factors, default_value,
                                         prefilter, n=n, **kwargs)
        self.ncrops = n
        self.grid_size = segment_size if grid_size is None else grid_size
        self.strides = self.grid_size if strides is None else strides
        self.nonzero = nonzero
    ...
    def _randomize(self):
        if self.n_ == 0:
            ...
            x = self.connections[0][0][0].shape[1] // 2
            y = self.connections[0][0][0].shape[2] // 2
            for z in list(range(starts[2], stops[2], self.strides[2])) + [stops[2]]:
                if self.nonzero:
                    if np.any(self.reference_connection[0][:, max(0, x):x + self.grid_size[0],
                              max(0, y):y + self.grid_size[1], max(0, z):z + self.grid_size[2], :]):
                        self.coordinates.append((x, y, z + self.grid_size[2] // 2))
